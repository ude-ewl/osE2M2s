...
The model has not been pre-run and must be executed manually before analysis. Before running, you must manually adjust the input and output paths at the beginning of the file <kbd>E2M2s-SRE_run.gms</kbd>. Specifically, replace the placeholder paths assigned to the variables PATH_IN_DATA and PATH_OUT with the full paths to your local input and output directories.
