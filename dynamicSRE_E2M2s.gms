...
Parameters
SRE_discrete(time) "rebound share in PV-free hours according to the discrete SRE definition"
pvAVG(zone) "provides the normalized/avareged PV generation profile used to distribute sweeping SRE demand over the day"

share_privatePV(zone) "percentage of PV installed on private households"
share_PVHH(zone) "share of households owning PV"
number_privateHH(zone) "number of private households (diagnostics)"
share_privateHH_totalELdemand(zone) "Share of household electricity in total electricity demand"
...
out_demand_y_SRE_disc(simyear,zone,product) "annual SRE increment occurring in PV-free hours (discrete component)"
out_demand_y_SRE_marg(simyear,zone,product) "Annual SRE increment coincident with PV output (marginal/simultaneous)"
out_trans_shadow(simyear, zone, zzone) "Shadow value of the electricity transmission restriction “eq_transpo_CF” [€/MW]"
;

Positive Variables
share_sweSRE(zone,time) "dynamic share of SRE sent to PV-free hours (off-peak component)"
share_simSRE(zone,time) "dynamic share of SRE coincident with PV output (midday component)"
;
*---------------------------- For SRE (MD) --------------------------------------------
...
*---------------------------- For SRE (MD) --------------------------------------------
eq_share_sweSRE(zone, time) "determines the share of SRE allocated to PV-free (off-peak) hours"
eq_share_simSRE(zone, time) "determines the share of SRE coinciding with PV generation (simultaneous component)"
;
*---------------------------- For SRE (MD) --------------------------------------------
...
*---------------------------- For SRE (MD) --------------------------------------------    
*Compute the dynamic split between sweeping (off-peak) and simultaneous (PV-coincident) SRE components
eq_share_sweSRE(zone,time).. share_sweSRE(zone,time) =e=
    
                (demand(time, zone, 'electricity') * SRE_discrete(time) * share_privateHH_totalELdemand(zone) * share_PVHH(zone))

    /
  
                ((demand(time, zone, 'electricity') * SRE_discrete(time) * share_privateHH_totalELdemand(zone) * share_PVHH(zone))
    + sum((heat_regio, exist_plant(power_plant, heat_regio), node)$(heatregio_in_zone(heat_regio, zone) AND power_plant_type(power_plant, 'SUN')),
                (cap_n_sunk(exist_plant) + cap_ref(exist_plant)) * prob_node(node) * PV(heat_regio, node, time) * SRE_effect_strength *  share_privatePV(zone) ))
    ;
    
*The sweeping share increases during PV-free hours, while the simultaneous share dominates during high PV generation
eq_share_simSRE(zone,time).. share_simSRE(zone,time) =e= 1 - share_sweSRE(zone,time)
;   
...
*Adds additional demand in alignment with PV generation, concentrating load around midday.            
        ...
        * PV(heat_regio, node, time) * SRE_effect_strength *  share_privatePV(zone) * share_simSRE(zone,time)) 
        
*Sweeping SRE
*Spreads additional demand evenly across the day based on an averaged PV profile.   
        + (demand(time, zone, 'electricity') * SRE_discrete(time) * share_privateHH_totalELdemand(zone) * share_PVHH(zone) * share_sweSRE(zone,time))
...
*Sweeping SRE        
        + sum((heat_regio, exist_plant(power_plant, heat_regio))$(heatregio_in_zone(heat_regio, zone) AND power_plant_type(power_plant, 'SUN')),
                (cap_n_sunk(exist_plant) + cap_ref(exist_plant))
        *  pvAVG(zone) * SRE_effect_strength *  share_privatePV(zone) * share_sweSRE(zone,time))
;
*---------------------------- For SRE (MD) --------------------------------------------
...
*The sweeping share (eq_share_sweSRE) and simultaneous share (eq_share_simSRE) jointly determine the temporal distribution of additional electricity demand
eq_share_sweSRE
eq_share_simSRE
*---------------------------- For SRE (MD) --------------------------------------------
/;
...
*Compute zone-averaged (normalized) PV profile used as helper for distributing sweeping SRE demand across time steps
*The weighting uses prob_node, hour_resolution, and freq_time to reflect the time-segmentation scheme
pvAVG(zone) =
    sum( (heat_regio,node,time1) $ (heatregio_in_zone(heat_regio,zone) and node_time(node,time1) ),
            prob_node(node) * hour_resolution(time1) * freq_time(time1) * PV(heat_regio, node, time1) )
  
    /
  
    sum( (node,time1)$node_time(node,time1), prob_node(node) * hour_resolution(time1) * freq_time(time1) );
*---------------------------- For SRE (MD) --------------------------------------------
...
*---------------------------- For SRE (MD) --------------------------------------------
*Estimate the share of households owning PV (share_PVHH):
*The factor 0.007 corresponds to an average system size of 7 kW per private household
*Multiplication with share_privatePV allocates only the household PV share
*Division by number_privateHH normalizes the result per household
    share_PVHH(zone) =
            sum((heat_regio, exist_plant(power_plant, heat_regio))$(heatregio_in_zone(heat_regio, zone) AND power_plant_type(power_plant, 'SUN')),
                    (cap_n_sunk(exist_plant) + cap_ref(exist_plant)) / 0.007 * share_privatePV(zone) / number_privateHH(zone))
;
*---------------------------- For SRE (MD) --------------------------------------------
...
*Simultaneous SRE
                                        ...
                                        * SRE_effect_strength * share_privatePV(zone) * share_simSRE.l(zone,time)) 
*Sweeping SRE        

                                        + (demand(time, zone, 'electricity') * SRE_discrete(time) * share_privateHH_totalELdemand(zone) * share_PVHH(zone) * share_sweSRE.l(zone,time))))  
...
;
*---------------------------- For SRE (MD) --------------------------------------------
        out_demand_y_SRE_marg(simyear, zone, 'electricity') = sum((node,time)$node_time(node, time), prob_node(node) * hour_resolution(time) * freq_time(time) *
*Simultaneous SRE
                                        ( sum((heat_regio, exist_plant(power_plant, heat_regio))$(heatregio_in_zone(heat_regio, zone) AND power_plant_type(power_plant, 'SUN')),
                                        (cap_n_sunk(exist_plant) - v_cap_new.l(exist_plant) + cap_ref(exist_plant)) * PV(heat_regio, node, time)
                                        * SRE_effect_strength * share_privatePV(zone) * share_simSRE.l(zone,time))))
;
        ...
        out_demand_y_SRE_disc(simyear, zone, 'electricity') = sum((node,time)$node_time(node, time), prob_node(node) * hour_resolution(time) * freq_time(time) * 
*Sweeping SRE        
                                        demand(time, zone, 'electricity') * SRE_discrete(time) * share_privateHH_totalELdemand(zone) * share_PVHH(zone) * share_sweSRE.l(zone,time))
;        
*---------------------------- For SRE (MD) --------------------------------------------
* Fig. 5: Export routine for transmission congestion costs: aggregates shadow values of eq_transpo_CF over time and nodes
