...
ax0 = axes[0]
ax0.plot(years, ref_demand, label="Reference", color="#666666", linestyle="-", marker='o', linewidth=2, markersize=5)
ax0.plot(years, high_swe_total, label="Maximum SRE", color=colors["SWE"], linestyle="--", marker='o', linewidth=2, markersize=5)
...
ax0.set_ylabel("Total electricity demand [TWh/yr]", fontweight='bold', fontsize=14)
ax0.set_xticks(years)
ax0.legend(frameon=False, loc="upper left")

for ax, (title, data) in zip(axes[1:], sre_demand.items()):
...
axes[1].set_ylabel("Additional electricity demand [TWh/yr]", fontweight='bold', fontsize=14)
...
handles = [plt.Line2D([], [], color=colors[k], linestyle=linestyles[k], marker=markers[k],
                      label=scenario_labels[k]) for k in ["DYN", "SIM", "SWE"]]
labels = [scenario_labels[k] for k in ["DYN", "SIM", "SWE"]]

fig.subplots_adjust(left=0.07, right=0.82)

axes[1].set_ylabel("Additional electricity demand [TWh/yr]", fontweight='bold', fontsize=14)
axes[1].spines['left'].set_visible(True)
axes[1].tick_params(axis='y', labelleft=True)

leg = fig.legend(handles, labels, loc='center left', bbox_to_anchor=(0.825, 0.5), frameon=False)
