...
for scenario in scenarios:
    for idx, year in enumerate(years):
        ...
        for neg_idx, neg_tech in enumerate(neg_tech_labels):
            ...
            ax1.bar(
                current, value, width=width, bottom=bottom_neg,
            ...
            if abs(value) >= 35:
                if abs(value) < 50:
                    ...
                    ax1.text(
                        current + offset, center_y, f"{int(value)}",
        ...
        for tech_idx, tech in enumerate(technologies):
            ...
            if value <= 0:
                continue
            ax1.bar(
                current, value, width=width, bottom=bottom_pos,
...
nY = len(years)
scenario_midpoints = [
    (x_positions[i * nY] + x_positions[i * nY + (nY - 1)]) / 2
    for i in range(len(scenarios))
]
for i, mid in enumerate(scenario_midpoints):
    t = ax1.text(
        mid, 1.02, scenario_labels[i],
...
for scenario in scenarios:
    ...
    for idx, year in enumerate(years):
        ...
        for neg_idx, neg_tech in enumerate(neg_tech_labels):
            ...
            if diff_val > 0:
                ax3.bar(current, diff_val, width=width, bottom=pos_bottom,
                        color=color, edgecolor="none")
                stacked_values.append((pos_bottom, pos_bottom + diff_val, True, neg_idx))
                pos_bottom += diff_val
            else:
                ax3.bar(current, diff_val, width=width, bottom=neg_bottom,
                        color=color, edgecolor="none")
