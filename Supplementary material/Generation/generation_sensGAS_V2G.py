...
absolute_data = {
    'full_avail_ref': [
        [315, 1365, 948, 540, 2, 21, 46, 502, 44, 966, 77],
        [317, 1611, 1148, 942, 19, 44, 59, 501, 55, 875, 68],
        [309, 1797, 1338, 1352, 39, 73, 67, 498, 63, 776, 8],
        [354, 1978, 1526, 1547, 62, 106, 141, 498, 64, 627, 4],
        [346, 2143, 1763, 1737, 52, 157, 164, 497, 67, 426, 2]
    ],
    'avg_avail': [
        [268, 1324, 971, 551, 2, 10, 46, 501, 51, 972, 76],
        [304, 1593, 1152, 954, 19, 38, 59, 501, 54, 875, 67],
        [308, 1788, 1330, 1366, 40, 66, 67, 498, 59, 776, 8],
        [354, 1969, 1518, 1557, 62, 100, 139, 498, 63, 628, 4],
        [343, 2132, 1768, 1748, 52, 156, 163, 496, 67, 427, 2]
    ],
    'low_avail': [
        [96, 1226, 1046, 595, 3, 34, 48, 501, 79, 1043, 68],
        [161, 1483, 1202, 998, 24, 51, 61, 500, 88, 886, 67],
        [213, 1719, 1353, 1411, 39, 90, 68, 498, 81, 778, 8],
        [213, 1895, 1570, 1602, 63, 152, 154, 498, 74, 627, 4],
        [202, 2069, 1840, 1792, 52, 229, 177, 496, 79, 425, 2]
    ]
}
    ...
    for idx, year in enumerate(years):
        ...
        for tech_idx, tech in enumerate(technologies):
            ...
            if diff_value > 2:
                ...
                always_inside = tech in ['Residential (rooftop) PV', 'Utility-scale PV', 'Onshore wind power', 'Offshore wind power','Battery storage', 'Hydrogen', 'Other']
                show_label = always_inside or (abs(diff_value) >= 2)
                if show_label:
                    label = f"{int(diff_value)}"
