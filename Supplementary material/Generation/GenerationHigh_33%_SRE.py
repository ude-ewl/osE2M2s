...
nY = len(years)
scenario_midpoints = [
    (x_positions[i * nY] + x_positions[i * nY + (nY - 1)]) / 2.0
                ...
                if tech != "Other":
                    ...
                    if diff_value > 2 or tech == "Battery storage":
                        ...
                        ax2.plot(
                            [
                                current
                                + (width / 2 if label_toggle_pos else -width / 2),
                                current + (0.3 if label_toggle_pos else -0.3),
                            ],
                            [center_y, center_y],
                            color="black",
                ...
                if show_label:
                    ...
                    if always_inside or abs(diff_value) > 15:
                        ...
                        ax2.plot(
                            [
                                current
                                + (width / 2 if label_toggle_neg else -width / 2),
...
for scenario in scenarios:
    ...
    for idx, _year in enumerate(years):
        ...
        for i, (bottom, top, is_pos, _tech_idx) in enumerate(stacked_values):
            diff_val = top - bottom
            if abs(diff_val) <= 1:
                continue
            ...
            ax3.plot(
                [
                    current + (width / 2 if ha == "left" else -width / 2),
...
pos_handles = [
    mpatches.Patch(color=colors[i], label=technologies[i])
    for i in range(len(technologies))
]
neg_handles = [
    mpatches.Patch(color=neg_colors[i], label=neg_tech_labels[i])
    for i in range(len(neg_tech_labels))
]
handles = pos_handles[::-1] + neg_handles

fig.subplots_adjust(right=0.83, top=0.93)
leg = ax1.legend(
    handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5), frameon=False
)
