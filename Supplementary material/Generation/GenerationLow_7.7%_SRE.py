...
neg_colors = ["#57C35E", "#CA41B1", "#5555ffff", "#E47B38"]
...
absolute_data = {
    ...
    "DYN": [
        [393, 677, 842, 525, 6, 28, 255, 70, 501, 1447, 92],
        [514, 855, 1124, 932, 33, 60, 286, 72, 499, 1166, 90],
        [602, 976, 1364, 1346, 67, 130, 366, 103, 498, 1002, 8],
        [693, 1098, 1686, 1536, 78, 218, 514, 105, 498, 754, 5],
        [768, 1176, 1840, 1726, 63, 295, 514, 94, 498, 764, 3],
    ],
    "SIM": [
        [393, 677, 841, 525, 6, 28, 254, 69, 501, 1448, 93],
        [514, 855, 1118, 932, 32, 58, 286, 71, 499, 1167, 90],
        [601, 974, 1345, 1346, 68, 128, 365, 102, 498, 1003, 8],
        [692, 1099, 1681, 1536, 78, 214, 510, 104, 498, 754, 5],
        [765, 1171, 1832, 1725, 63, 289, 510, 94, 498, 765, 3],
    ],
    "SWE": [
        [387, 664, 846, 525, 6, 29, 256, 70, 501, 1447, 92],
        [508, 839, 1138, 933, 34, 62, 289, 73, 499, 1164, 90],
        [596, 962, 1381, 1346, 66, 131, 366, 106, 498, 1000, 8],
        [680, 1082, 1720, 1544, 78, 215, 514, 107, 498, 753, 5],
        [754, 1157, 1858, 1735, 63, 294, 517, 95, 498, 764, 3],
    ],
}
...
difference_data: dict[str, list[list[float]]] = {}
for scen in scenarios:
    if scen == "REF":
        continue
    diff_years: list[list[float]] = []
    for yi in range(n_years):
        diff_year: list[float] = []
        for ti in range(n_techs):
            diff_year.append(absolute_data[scen][yi][ti] - absolute_data["REF"][yi][ti])
        diff_years.append(diff_year)
    difference_data[scen] = diff_years

charging_diff_data: dict[str, list[list[float]]] = {}
for scen in scenarios:
    if scen == "REF":
        continue
    diff_years = []
    for yi in range(n_years):
        ...
        for ci in range(n_neg):
            ...
            diff_year.append(-(scen_val - ref_val))
        diff_years.append(diff_year)
...
for scenario in scenarios:
    for idx, year in enumerate(years):
        ...
        for neg_idx, neg_tech in enumerate(neg_tech_labels):
            value = float(charging_data[scenario][idx][neg_idx])
            ax1.bar(
                current,
                value,
                width=width,
            ...
            if abs(value) >= 35:
                if abs(value) < 50:
                    ...
                    ax1.plot(
                        [
                            current + (width / 2 if label_toggle_neg else -width / 2),
        ...
        for tech_idx, tech in enumerate(technologies):
            ...
            if label_allowed:
                if (
                    (tech == "Other" and value >= 7)
                    or tech == "Battery storage"
                    or tech == "Pumped hydro storage"
                    or (tech == "Biomass" and year in [2040, 2045, 2050])
                    or (tech != "Other" and value > 150)
                ):
                    ax1.text(
                        current,
                        center_y,
                    ...
                    ax1.plot(
                        [
                            current + (width / 2 if label_toggle_pos else -width / 2),
...
nY = len(years)
scenario_midpoints = [
    (x_positions[i * nY] + x_positions[i * nY + (nY - 1)]) / 2.0 for i in range(len(scenarios))
]
...
for scenario in scenarios:
    ...
    for idx, _year in enumerate(years):
        ...
        neg_toggle = True
        stacked_values = []
        for neg_idx, _neg_tech in enumerate(neg_tech_labels):
            diff_val = float(charging_diff_data[scenario][idx][neg_idx])
            if diff_val == 0:
                continue
            color = neg_colors[neg_idx]
            if diff_val > 0:
                ax3.bar(current, diff_val, width=width, bottom=pos_bottom, color=color, edgecolor="none")
                stacked_values.append((pos_bottom, pos_bottom + diff_val, True, neg_idx))
                pos_bottom += diff_val
            else:
                ax3.bar(current, diff_val, width=width, bottom=neg_bottom, color=color, edgecolor="none")
        ...
        for i, (bottom, top, is_pos, tech_idx) in enumerate(stacked_values):
            ...
            if not overlapping:
                ax3.text(current, y_pos, value_str, ha="center", va="center", fontsize=8, fontweight="bold")
                continue
            if is_pos:
            ...
            ax3.text(current + offset, y_pos, value_str, ha=ha, va="center", fontsize=8, fontweight="bold")
            ax3.plot(
                [current + (width / 2 if ha == "left" else -width / 2), current + (0.25 if ha == "left" else -0.25)],
                [y_pos, y_pos],
                color="black",
                linewidth=0.5,
            )
...
ax1.set_xticks([])
ax2.set_xticks([])
ax1.set_ylabel("Charging/pumping & production (TWh/yr)")
ax2.set_ylabel("Difference to the reference —\nproduction (TWh/yr)")
ax3.set_ylabel("Difference to the reference —\ncharging/pumping (TWh/yr)")
...
fig.subplots_adjust(right=0.83, top=0.93)
leg = ax1.legend(handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5), frameon=False)
