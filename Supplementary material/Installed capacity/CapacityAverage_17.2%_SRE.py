            ...
            if value > 0:
                ...
                if tech in {"Hydrogen", "Battery storage", "Electrolyzer"}:
                    ...
                    if year in {2030, 2035}:
                        ...
                        ax1.plot(
                            [current + (width / 2 if label_toggle_abs else -width / 2),
                    ...
                    if (
                        ...
                        or (tech in {"Pumped hydro storage"})
...
for i, lbl in enumerate(scenario_labels):
    left = x_positions[i * nY]
    right = x_positions[i * nY + (nY - 1)]
    mid = 0.5 * (left + right)
    t = ax1.text(
        mid, 1.02, lbl,
    ...
    for idx, _year in enumerate(years):
        ...
        for tech_idx, _tech in enumerate(technologies):
            ...
            if diff_value > 2:
                ...
                ax2.text(current, center_y, f"+{int(diff_value)}",
...
ax2.set_xticklabels([str(y) for y in years] * len(scenarios))
ax2.set_xlabel("Year")
...
fig.subplots_adjust(left=0.08, right=0.83, top=0.92, bottom=0.12)
ax1.legend(handles=handles, loc="center left", bbox_to_anchor=(1.01, 0.5), frameon=False, fontsize=8)
